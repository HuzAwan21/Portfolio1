...
class RV32I(
  ...
  switch(control_unit.io_ctrl.next_pc_select) {
    ...
    is(NEXT_PC_SELECT.ALU_OUT_ALIGNED) {
      when(RISCV_TYPE.getOP(instr_type) === RISCV_OP.JALR) {
        io_pc.pc_wdata := alu.io_alu.result(31, 1) ## Fill(1, 0.U)
      } .otherwise {
        io_pc.pc_wdata := alu.io_alu.result //  & "hFFFF0000".U // & BigInt("FFFF0000", 16).U  or ## Fill(16, 0.U) or & (BigInt("FFFF0000", 16.U)
      }
    }
  }
  ...
  switch(control_unit.io_ctrl.reg_write_sel) {
    ...
    is(REG_WRITE_SEL.MEM_OUT_SIGN_EXTENDED) {
      val func3 = (RISCV_TYPE.getFunct3(control_unit.io_ctrl.instr_type))
      switch (func3) {          
    ...
    is(REG_WRITE_SEL.MEM_OUT_ZERO_EXTENDED) {
      ...
      switch (func3) {
          is (RISCV_FUNCT3.F100) { // lbu
            io_reg.reg_write_data := Fill(24, 0.U) ## io_data.data_rdata(7, 0)
          }
          is (RISCV_FUNCT3.F101) { // lhu
            io_reg.reg_write_data := Fill(16, 0.U) ## io_data.data_rdata(15, 0)
          }
      }
    }
  }
