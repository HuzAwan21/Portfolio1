...
import RISCV.model.REG_WRITE_SEL.ALU_OUT
...
class ControlUnit extends AbstractControlUnit {
  ...
  when(was_stalled === STALL_REASON.EXECUTION_UNIT) {
    ...
    switch(RISCV_TYPE.getOP(io_ctrl.instr_type)) {
      ...
      is (RISCV_OP.LOAD) {
        stalled := STALL_REASON.EXECUTION_UNIT          // KEINE AHNUNG was die stall.execution unit macht
        io_ctrl.reg_we := false.B                  // müssen kein register beschreiben, da wir AUS EINEM REGISTER laden
        
        // switch for mem sign or zero extended for reg_write_sel (actually not sure if we need it here)
        ...
        io_ctrl.data_req := true.B
        io_ctrl.data_we := false.B                // do not write to memory

        io_ctrl.next_pc_select := NEXT_PC_SELECT.PC_PLUS_4        // PC wird ganz normal erhöht auf nächste Instruction
      }
    }
  }
}
