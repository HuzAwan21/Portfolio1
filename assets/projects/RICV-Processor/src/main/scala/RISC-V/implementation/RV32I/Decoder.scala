...
class Decoder extends AbstractDecoder {
  ...
  switch(opcode) {
    ...
    is(RISCV_OP.JAL) {
      ...
      io_decoder.rs2 := 0.U
      io_decoder.rd := RD
      val immediate = Fill(11, io_decoder.instr(31)) ## io_decoder.instr(31) ## io_decoder.instr(19, 12) ## io_decoder.instr(20) ## io_decoder.instr(30, 21) ## 0.U
      io_decoder.imm := immediate
    }
    is(RISCV_OP.STORE) {
      io_decoder.rs1 := RS1
      io_decoder.rs2 := RS2
